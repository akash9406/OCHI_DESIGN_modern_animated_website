...
const Navbar = () => {
  ...
  const [lastScrolY, setLastScrolY] = useState<number>(0);

  const controlNavbar = () => {
    if (typeof window !== "undefined") {
      if (window.scrollY > lastScrolY) {
        setShow(false);
      } else {
        setShow(true);
      }
  ...
  useEffect(() => {
    if (typeof window !== undefined) {
      ...
      return () => {
        window.removeEventListener("scroll", controlNavbar); //cleanup function
      };
    }
  }, [lastScrolY]);
  return (
    <div
      className={`fixed z-[999] w-full flex  transition-transform transform ${
        show ? "translate-y-0" : "-translate-y-full"
      } backdrop-filter backdrop-blur-sm  px-12 py-6 font-['Neue Montreal'] justify-between items-center `}
    >
      <div className="logo">
        <svg
          width="72"
      ...
      <div className="link flex gap-10 ">
        <a
          className="text-md cursor-pointer capitalize font-regular relative group  
          after:content-['']
          after:absolute
          after:bg-zinc-800
          after:h-[1px]
          ...
          after:delay-200
          hover:after:w-[100%]
          [&:not(:hover)]:after:right-[0]
          [&:not(:hover)]:after:left-[auto]
          overflow-hidden
        "
        >
          <span
            className="relative  
             top-0
             group-hover:top-[-100%]
             transition-all duration-[0.2s] ease-linear
          ...
          <span
            className="absolute top-[100%] left-[0] group-hover:translate-y-[-100%] 
          transition duration-[0.2s]  ease-linear  "
          >
            Servics
          </span>
        </a>
